    ...
    resources: new (class extends ResourceLoader {
      ...
      fetch(url, options) {
        if (resourceLoader) {
          ...
          if (resource) {
            const promise = resource.then((content) => Buffer.from(content));

            const abortable = /** @type {import("jsdom").AbortablePromise<Buffer>} */ (promise);
            /** @type {import("jsdom").AbortablePromise<Buffer>} */ (promise).abort = () =>
              undefined;
